...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function fitRelevantTerminalsDebounced() {
    ...
    fitTimeout = setTimeout(() => {
      const fullscreenWrapper = terminalsContainer.querySelector(
      ...
      if (fullscreenWrapper) {
        ...
        Object.values(terminals).forEach((termInfo) => {
          if (termInfo.term) fitTerminal(termInfo);
        });
      }
    }, 50);
  }
  ...
  function setTerminalMinWidth(width) {
    ...
    terminalsContainer.style.setProperty(
      "--terminal-min-width",
      `${safeWidth}px`,
    );
    if (document.activeElement !== termWidthInput)
      termWidthInput.value = safeWidth;
  ...
  function updateGlobalControlsState() {
    ...
    const canReceiveInput = Object.values(terminals).some(
      (t) => t.isReady && !t.isClosed && !t.hasError,
    );
    commandInput.disabled = !groupSelected || !canReceiveInput;
    commandInput.placeholder = !groupSelected
      ? "Select group first"
      : !canReceiveInput
  ...
  socket.on("disconnect", (reason) => {
    ...
    reconnectGroupBtn.disabled = true;
    clearAllTerminals();
    setConnectionStatus(
  ...
  termWidthInput.addEventListener("change", (event) =>
    setTerminalMinWidth(event.target.value),
  );
  ...
  terminalsContainer.addEventListener("click", (event) => {
    ...
    if (lockBtn) {
      ...
      terminalsContainer.classList.toggle(
